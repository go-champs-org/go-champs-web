describe('getTeamStatsLogsByGame', () => {
  ...
  it('filters the logs by game_id and maps the response', async () => {
    ...
    jest.resetModules();
    const { getTeamStatsLogsByGame } = await import('./teamStatsLogs');
    ...
    const result = await getTeamStatsLogsByGame('g1');

    expect(global.fetch).toHaveBeenCalledWith(
      'https://api.example.com/v1/team-stats-logs?where%5Bgame_id%5D=g1',
